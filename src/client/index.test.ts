...
import { WorkOSAuthKit } from "./index.js";
...
export const testQuery = query({
  ...
  handler: async (ctx, args) => {
    return await workOSAuthKit.count(ctx, args.name);
  },
});
...
export const testMutation = mutation({
  ...
  handler: async (ctx, args) => {
    return await workOSAuthKit.add(ctx, args.name, args.count);
  },
});
...
describe("WorkOSAuthKit thick client", () => {
  beforeEach(async () => {
    vi.useFakeTimers();
  });
  afterEach(() => {
  ...
  test("should make thick client", async () => {
    const c = new WorkOSAuthKit(components.workOSAuthKit);
