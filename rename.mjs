...
async function setup() {
  ...
  const replacements = [
    // NPM package name
    ["@convex-dev/workos-authkit", npmPackageName],

    // Repository name
    ["get-convex/workos-authkit", repoName],

    // Component name variations
    ["WorkOSAuthKit", cases.pascal],
    ["workOSAuthKit", cases.camel],
    ["work-os-auth-kit", cases.kebab],
    ["work_os_auth_kit", cases.snake],
    ["work os auth kit", cases.space],
    ["Work Os Auth Kit", cases.title],
  ];
  if (npmPackageName.includes("/")) {
    replacements.push([
      "@example%2Fwork-os-auth-kit",
      npmPackageName.replace("/", "%2F"),
    ]);
  } else {
    replacements.push(["@example%2Fwork-os-auth-kit", npmPackageName]);
  }
